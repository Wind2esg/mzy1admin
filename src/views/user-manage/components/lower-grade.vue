...
<div class="agent_warper">
  <Modal v-model="isShow" width=800 :title='title' @on-cancel="Modal_cencel" class="agent_modalwarp">
      <div class="agent_modalWraper">
        ...
        <Select v-model="RoleId" class="selectType" placeholder="选择代理等级">
          <Option v-for="(item,index) in lmlRoleList" :value="item.id" :key="index">{{ item.roleName }}</Option>
        </Select>
        ...
        <Page :total="total" @on-change="pageChange" :current="pageIndex"></Page>
      </div>
  ...
  <Modal
      v-model="info_model"
      width=460
      title="修改上级" class="agentInfo_model" @on-cancel="info_model_cancel">
      <Form ref="formInfo" :model="formInfo" :rules="ruleInfo" :label-width="80">
        <FormItem prop="phone" label="上级电话">
      ...
      <div slot="footer">
        <Button size="large" @click="info_model_cancel">关闭</Button>
        <Button type="primary" :loading="info_loading" size="large" @click="info_model_ok" >确认</Button>
      </div>
  </Modal>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    exportDocument_click() {
      ...
      let url = this.$api.exportLowMemberInfoList;
      let initUrl = url
    ...
    GetList(callback){
      ...
      return that.$request(this.$api.getLowMemberInfoList, requedata, data => {
    ...
    //弹框确认 == 批量修改
    info_model_ok(){
      if(that.formInfo.name){
        that.info_loading = true;
        this.resultList = [];
        let requedata = {
          SuperId:that.result.memberId,
  ...
  watch: {
    visible(value) {
      ...
      if(value){
        ...
        let lmlRoleList = []
        console.log(that.lmlRoleId);
        that.RoleList.map((el, ix) => {
          if(el.id>that.lmlRoleId) lmlRoleList.push(el)
        })
        that.lmlRoleList = lmlRoleList;
        if (that.lmlRoleList.length == 1){
          that.RoleId = that.lmlRoleList[0].id;
        }
        return that.GetList()
      }
    }
