...
<div class="agent_warper">
  <Modal v-model="isShow" width=900 title='代理信息' @on-cancel="cancel_fn" class="agent_modalwarp">
      ...
      <Row class="Modal_Info_row" type="flex" justify="center" align="middle">
        ...
        <Col span="8">
          <Row class="">
            ...
            <Col span="14" class="Info_cont">{{fromInfo.authPayType + '('+fromInfo.authPayChannel+')'}}</Col>
          </Row>
        </Col>
        <Col span="8">
          <Row class="" >
  ...
  <Modal
      v-model="veto_model"
      width=460
      title="否决原因" @on-cancel="veto_model=false;formVeto.RefuseReason=''">
      ...
      <div slot="footer">
        <Button size="large" @click="veto_model=false;formVeto.RefuseReason=''">关闭</Button>
...
export default {
  ...
  watch: {
    visible(value) {
      ...
      if(value){
        let that = this
        return that.$request(this.$api.getCheckMemberInfoDetails, {MemberId:that.MemberId}, data => {
          console.log(data,66);
          that.fromInfo = data.result;
          if(!data.isSuccess) that.$Message.info(data.message);
        })
      }
    }
  }
