...
export default {
    ...
    methods: {
        getData(memberId, callback){
            const params = {
                RecommandId: memberId
            }
            this.$request(this.$api.GetRecommandStruct, params, data=>{
        ...
        filterData(data){
            return data.result.map(item=>{
                let obj = {
                    title: item.roleName+'：'+item.name+'(手机号:'+item.phone+(item.isTeamLeader?'，团队长':'')+')'+(item.recommandCount ? '('+item.recommandCount+')' : ''),
                    memberId: item.memberId
                };
                if (item.recommandCount){
                    obj.loading = false;
                    obj.children = [];
                }
                ...
                this.searchList.map(record=>{
                    if (record.recommandId === item.memberId){
                        haveId = true;
                    }
                    isLast = record.recommandId === item.memberId;
                })
        ...
        search_click(){
            ...
            this.$request(this.$api.GetRecommandStructByKeyWords, params, data=>{
                //this.data = this.filterData(data);
                if (data.isSuccess){
        ...
        exportDocument_click(){
            ...
            let url = this.$api.ExportRecommandStruct;
            url += '?RecommandId='+this.selectId;
            location.href = url;
        }
    }
};
