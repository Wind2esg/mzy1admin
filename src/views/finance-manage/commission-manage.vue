...
<template>
    <Card class="saled-order">
        <div class="search">
            ...
            <DatePicker type="date" v-model="date" placeholder="申请时间（最大时间）" style="width: 200px;vertical-align:top"></DatePicker>
        ...
        <Modal v-model="visible" width="360" @on-cancel="modalCancel">
            ...
            <Form ref="form" :model="form" :rules="rule" :label-width="80">
                <FormItem :label="(rechargeType==1?'充值':'扣款')+'金额'" prop="price">
                    <InputNumber v-model="form.price" :min=0 style="width:200px"></InputNumber>
                </FormItem>
...
export default {
    ...
    data () {
        return {
            ...
            columns: [
                ...
                {
                    title: '提现中佣金',
                    key: 'drawingCommission'
                },
                {
                    title: '已提现佣金',
                    key: 'drawedCommission'
                },
                {
                    title: '操作',
                    width: 160,
    ...
    methods: {
        getData(){
            ...
            this.$request(this.$api.GetMemberCommissionManagerList, params, data=>{
        ...
        saveData(){
            ...
            let url = this.$api.PutUpCommission;

            let params = {
                CommissionMoney: this.form.price,
                MemberId: this.rechargeData.memberId,
                Reason: this.form.reason
            }

            if (this.rechargeType == 2){
                url = this.$api.PutDeductCommission;
            }
        ...
        exportData(){
            let url = this.$api.ExportMemberCommissionManagerList + '?' + ['KeyWords='+this.keywords, 'RoleId='+this.level, 'AddTime='+(this.date==''?'':this.format(this.date))].join("&");
            location.href = url;
        }
    }
};
